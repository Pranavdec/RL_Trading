...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, df_option_data, window_size, frame_bound, margin, lot_size, spread, options_path = None):
        assert df.ndim == 2
        ...
        # spaces 
        self.action_space = gym.spaces.Discrete(4)  # Discrete actions: 4 choices (0, 1, 2, 3) 0 long, 1 short, 2 hold, 3 close 

        stock_data_size = self.window_size * self.signal_features.shape[1]
        strike_prices_size = self.window_size * 9
        portfolio_info_size = 35 * 9
        observation_space_shape = (stock_data_size + strike_prices_size + portfolio_info_size,)

        self.observation_space = gym.spaces.Box(
            low=-np.inf, high=np.inf, shape=observation_space_shape, dtype=np.float64
        )

        self.trade_fee_buy_percent = 0.01
        self.trade_fee_sell_percent = 0.005
        self.spread = spread # no.of strikes away from ATM below and above
    ...
    def step(self, action):
        ...
        # if action == 4:
        #     self.spread -= 1
        #     self.spread = max(self.spread, 0)
        # elif action == 5:
        #     self.spread += 1
        #     self.spread = min(self.spread, len(strikes) - 1)
    ...
    def _process_strike_prices(self):
        ...
        x = x[1:y]
        list_1 = x.split(',')
        list_1 = [float(i) if i else 0.0 for i in list_1]
        list_1 = sorted(list_1)
        list_1 = [str(i).lstrip('0') for i in list_1]
        list_1 = [float(i) if i else 0.0 for i in list_1]
    ...
    def _add_to_portfolio(self, action, atm_strike, itm_strike, otm_strike, atm_price, itm_price, otm_price, expiry_date, date):
        ...
        self._portfolio.append((trade_type, atm_strike, itm_strike, otm_strike, atm_price, itm_price, otm_price, expiry_date, abs(margin)))
        self._history[date][trade_type] = (atm_strike, itm_strike, otm_strike, atm_price, itm_price, otm_price, expiry_date, abs(margin))
    ...
    def _get_observation(self):
        ...
        strike_prices = self._process_option_data_for_observation()

        portfolio_info = self._process_portfolio_info()

        observation = np.concatenate((
            current_features.flatten(),
            strike_prices.flatten(),
    ...
    def _process_option_data_for_observation(self):
        ...
        for i, date in enumerate(formatted_date_window[-self.window_size:]):
            if date in atm_data['FH_TIMESTAMP'].values:
                atm_info[i] = atm_data.loc[atm_data['FH_TIMESTAMP'] == date, ['FH_CLOSING_PRICE', 'FH_TOT_TRADED_QTY', 'Days_to_Expiry']].values[0]
            if date in itm_data['FH_TIMESTAMP'].values:
                itm_info[i] = itm_data.loc[itm_data['FH_TIMESTAMP'] == date, ['FH_CLOSING_PRICE', 'FH_TOT_TRADED_QTY', 'Days_to_Expiry']].values[0]
            if date in otm_data['FH_TIMESTAMP'].values:
                otm_info[i] = otm_data.loc[otm_data['FH_TIMESTAMP'] == date, ['FH_CLOSING_PRICE', 'FH_TOT_TRADED_QTY', 'Days_to_Expiry']].values[0]
    ...
    def _process_data(self):
        ...
        for key in features:
            features[key] = features[key][self.frame_bound[0]:self.frame_bound[1] + 1]

        prices = prices[self.frame_bound[0]:self.frame_bound[1] + 1]
    ...
    def _update_profit(self,action):
        ...
        if action == 3:
            for i in range(len(self._portfolio)):
                ...
                new_prices = [self.find_nearest_strikeprice_onexpiry(strike, datetime.strptime(current_date_str, '%Y-%m-%d')) for strike in strikes]
